		...
		queries.map<any>(q => {
			...
			switch (q.clinicalAttributeId) {
				...
				case SpecialAttribute.FractionGenomeAltered:
					...
					ret = Promise.all(
						Object.keys(studyToSamples).map(studyId => {
							const samplesInStudy = studyToSamples[studyId];
							if (samplesInStudy.length) {
				...
				default:
					ret = Promise.all([
						client.fetchClinicalDataUsingPOST({
							clinicalDataType: q.patientAttribute
								? "PATIENT"
								: "SAMPLE",
...
export default class ClinicalDataCache extends LazyMobXCache<
	| ClinicalData[]
	| MutationCount[]
	| FractionGenomeAltered[]
	| MutationSpectrum[],
