...
class LazyMobXTableStore<T> {
	...
	@computed
	get filterStringUpper() {
		return this.filterString.toUpperCase();
	}

	@computed
	get filterStringLower() {
	...
	@computed
	get colVisProp(): IColumnVisibilityDef[] {
	...
	@computed
	get paginationStatusText(): string {
	...
	@action
	setProps(props: LazyMobXTableProps<T>) {
	...
	@action
	public updateColumnVisibility(id: string, visible: boolean) {
