...
export default class StudyListLogic {
	...
	get map_node_filterBySearchText() {
		...
		for (let [node, meta] of this.store.treeData.map_node_meta.entries())
			map_node_searchResult.set(
				node,
				perform_search_single(parsedQuery, meta.searchTerms)
			);
	...
	@cached
	get map_node_filterBySelectedCancerTypes() {
		let map_node_filter = new Map<CancerTreeNode, boolean>();
	...
	@cached
	get map_node_filterBySelectedStudies() {
		let map_node_filter = new Map<CancerTreeNode, boolean>();
	...
	cancerTypeContainsSelectedStudies(cancerType: CancerType): boolean {
		...
		return (
			_.intersection(this.store.selectedStudies, descendantStudies)
				.length > 0
		);
	}
...
export class FilteredCancerTreeView {
	...
	getCheckboxProps(
		...
		if (meta.isCancerType) {
			...
			let selectedStudies = selectedStudyIds.map(
				studyId =>
	...
	@action
	onCheck(node: CancerTreeNode, checked: boolean): void {
		let clickedStudyIds;
		let meta = this.getMetadata(node);
	...
	private handleCheckboxStudyIds(
		...
		else selectedStudyIds = _.difference(selectedStudyIds, clickedStudyIds);
