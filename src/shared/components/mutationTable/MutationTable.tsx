...
export default class MutationTable<P extends IMutationTableProps> extends React.Component<P, {}> {
		...
		this._columns[MutationTableColumnType.FUNCTIONAL_IMPACT] = {
			...
			render: (d: Mutation[]) =>
				this.props.genomeNexusEnrichmentCache ? (
					FunctionalImpactColumnFormatter.renderFunction(d, this.props
						.genomeNexusEnrichmentCache as GenomeNexusEnrichmentCache)
				) : (
					<span />
				),
			download: (d: Mutation[]) =>
	...
	@computed
	protected get orderedColumns(): MutationTableColumnType[] {
