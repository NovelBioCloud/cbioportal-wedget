...
> {
	...
	private get tabs() {
		...
		const geneTabs = _.map(
			alterationCountsForCancerTypesByGene,
			(geneData, geneName: string) => {
				...
				return (
					<MSKTab
						key={geneName}
						id={"summaryTab" + geneName}
						linkText={geneName}
						anchorStyle={anchorStyle}
					>
						<CancerSummaryContent
							groupedAlterationData={
								alterationCountsForCancerTypesByGene[geneName]
							}
	...
	public render() {
		const isComplete =
			this.props.store.samplesExtendedWithClinicalData.isComplete &&
			this.props.store.alterationsByGeneBySampleKey.isComplete;
		const isPending =
			this.props.store.samplesExtendedWithClinicalData.isPending &&
