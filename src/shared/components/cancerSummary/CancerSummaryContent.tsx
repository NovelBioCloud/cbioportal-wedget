...
export class CancerSummaryContent extends React.Component<ICancerSummaryContentProps, {}> {
	...
	@observable
	private pdf: { anchor: string; width: number; height: number } = {
		anchor: "",
		width: 0,
		height: 0
	};
	...
	@computed
	get barChartTitle() {
		const type = _.find(GroupByOptions, {
	...
	get barChartDatasets(): IBarChartSortedData[] {
		...
		return _.chain(ret)
			.orderBy(i => i.sortCount, ["desc"])
			.value()
			.slice(0, CANCER_SUMMARY_BARLIMIT);
	}
	...
	private get cancerTypes() {
		...
		return [{ label: "All", value: "all" }, ...sortedCancerTypes];
	}
	...
	private get chartData() {
		...
		const orderedDatasets = _.orderBy(
			datasets,
						...
						<FormGroup>
							...
							<div className="slider custom-labels">
								<Slider
									...
									labels={{
										0: 0 + symbol,
										[altMax]: Math.ceil(altMax) + symbol
									}}
				...
				<Then>
					<div>
						...
						<SummaryBarGraph
							...
							setPdfProperties={this.setPdfProperties}
							setPngAnchor={this.setPngAnchor}
							legend={this.showGenomicAlt}
							title={this.barChartTitle}
							orderedLabels={orderedLabels}
							altCasesMax={this.altCasesMax}
						/>
					</div>
				</Then>
				<Else>
