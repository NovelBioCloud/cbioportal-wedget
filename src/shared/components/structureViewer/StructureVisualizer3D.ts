...
export default class StructureVisualizer3D extends StructureVisualizer {
	...
	@action
	setState(newState: IStructureVisualizerState) {
	...
	@computed
	get residuesToUpdate(): IResidueHelper[] {
		if (this.needToUpdateResiduesOnly) {
	 ...
	 */
	@computed
	get needToUpdateResiduesOnly(): boolean {
		return (
	...
	@computed
	get residuePositionsUnchanged(): boolean {
		return _.isEqual(
	...
	@computed
	get currentResidueToPositionMap(): { [residue: number]: IResidueHelper[] } {
		return generateResiduePosToSelectorMap(this.state.residues);
	}

	@computed
	get prevResidueToPositionMap(): { [residue: number]: IResidueHelper[] } {
		return generateResiduePosToSelectorMap(this._prevState.residues);
	}

	@computed
	get visualPropsUnchanged(): boolean {
		return _.isEqual(
