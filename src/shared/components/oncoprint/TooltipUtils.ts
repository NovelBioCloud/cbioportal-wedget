...
export function makeClinicalTrackTooltip(
	...
	return function(d: any) {
		...
		if (track.datatype === "counts") {
			...
			const attr_vals =
				(d.attr_val_counts && Object.keys(d.attr_val_counts)) || [];
		...
		ret += link_id
			? d.sample
				? sampleViewAnchorTag(d.study_id, d.sample)
				: patientViewAnchorTag(d.study_id, d.patient)
...
export function makeHeatmapTrackTooltip(
	...
	return function(d: any) {
		...
		ret += d.sample
			? link_id ? sampleViewAnchorTag(d.study, d.sample) : d.sample
			: link_id ? patientViewAnchorTag(d.study, d.patient) : d.patient;
		return $("<div>")
			.addClass(TOOLTIP_DIV_CLASS)
			.append(ret);
	};
	...
	function listOfMutationOrFusionDataToHTML(data: any[]) {
		return data.map(function(d: any) {
			...
			if (d.cancer_hotspots_hotspot) {
				ret.append(
					'<img src="images/cancer-hotspots.svg" title="Hotspot" style="height:11px; width:11px; margin-left:3px"/>'
				);
			}
			if (d.oncokb_oncogenic) {
				ret.append(
					'<img src="images/oncokb-oncogenic-1.svg" title="' +
						d.oncokb_oncogenic +
			...
			if (d.driver_tiers_filter) {
				ret.append(
					'<img src="images/driver_tiers.png" title="' +
						...
						'" alt="driver tiers filter" style="height:11px; width:11px;margin-left:3px"/>'
				);
			}
			return ret;
		});
				...
				case "COPY_NUMBER_ALTERATION":
					if (
						disp_cna.hasOwnProperty(
							(datum as GeneMolecularData).value
						)
					) {
						const tooltip_datum: any = {
