	...
	return remoteData<GeneticTrackSpec[]>({
		await: () => [
			...
			oncoprint.props.store
				.putativeDriverFilteredCaseAggregatedDataByOQLLine,
		...
		invoke: async () => {
			return oncoprint.props.store
				.putativeDriverFilteredCaseAggregatedDataByOQLLine.result!.map(
				(
					x: {
					...
					const sequenced = sampleMode
						? oncoprint.props.store.sequencedSampleKeysByGene
								.result![x.oql.gene].length
						: oncoprint.props.store.sequencedPatientKeysByGene
								.result![x.oql.gene].length;
					const altered = sampleMode
						? Object.keys(x.cases.samples).filter(
								k => !!x.cases.samples[k].length
							).length
						: Object.keys(x.cases.patients).filter(
								k => !!x.cases.patients[k].length
							).length;
...
export function makeClinicalTracksMobxPromise(
	...
	return remoteData<ClinicalTrackSpec[]>({
		...
		invoke: async () => {
			...
			await oncoprint.props.store.clinicalDataCache.getPromise(
				attributes,
				true
			);
			return attributes.map((attribute: ClinicalAttribute) => {
				...
				if (attribute.datatype === "NUMBER") {
					...
					if (
						attribute.clinicalAttributeId ===
						SpecialAttribute.FractionGenomeAltered
					) {
						(ret as any).numberRange = [0, 1];
					} else if (
						attribute.clinicalAttributeId ===
						SpecialAttribute.MutationCount
					) {
...
export function makeHeatmapTracksMobxPromise(
	...
	return remoteData<HeatmapTrackSpec[]>({
		...
		invoke: async () => {
			...
			await oncoprint.props.store.geneMolecularDataCache
				.result!.getPromise(cacheQueries, true);
			...
			return cacheQueries.map(query => {
				...
				const data = oncoprint.props.store.geneMolecularDataCache
					.result!.get(query)!.data!;
				return {
					...
					onRemove: action(() => {
						...
						if (trackGroup) {
							...
							if (!trackGroup.genes.size) {
								molecularProfileIdToHeatmapTracks.delete(
									molecularProfileId
								);
								if (
