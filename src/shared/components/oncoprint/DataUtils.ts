...
import {
	...
	isSample,
	isSampleList
} from "../../lib/CBioPortalAPIUtils";
import {
	getSimplifiedMutationType,
	SimplifiedMutationType
} from "../../lib/oql/accessors";
		...
		switch (molecularAlterationType) {
			...
			case "MUTATION_EXTENDED":
				let oncoprintMutationType = getOncoprintMutationType(
					getSimplifiedMutationType(event.mutationType)!
				);
...
export function makeGeneticTrackData(
	...
	if (isSampleList(cases)) {
		...
		for (const sample of cases) {
			...
			if (
				!sampleSequencingInfo.wholeExomeSequenced &&
				!sampleSequencingInfo.sequencedGenes.hasOwnProperty(
					hugoGeneSymbol
				)
			) {
				newDatum.na = true;
				...
				if (sampleSequencingInfo.sequencedGenes[hugoGeneSymbol]) {
					newDatum.coverage =
						sampleSequencingInfo.sequencedGenes[hugoGeneSymbol];
				}
		...
		for (const patient of cases) {
			...
			if (
				!patientSequencingInfo.wholeExomeSequenced &&
				!patientSequencingInfo.sequencedGenes.hasOwnProperty(
					hugoGeneSymbol
				)
			) {
				newDatum.na = true;
				...
				if (patientSequencingInfo.sequencedGenes[hugoGeneSymbol]) {
					newDatum.coverage =
						patientSequencingInfo.sequencedGenes[hugoGeneSymbol];
				}
			...
			for (const x of data) {
				if (isMutationCount(x)) {
					...
					const newVal = parseFloat(
						(x as ClinicalData | FractionGenomeAltered).value + ""
					);
					if (!isNaN(newVal)) {
						numValCount += 1;
						numValSum += newVal;
