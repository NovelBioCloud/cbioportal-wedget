...
export default class VirtualCohorts {
	public static delete(virtualCohortID: string) {
		let localStorageVirtualCohorts: LocalStorageVirtualCohort[] = JSON.parse(
			localStorage.getItem(VIRTUAL_COHORTS_LOCAL_STORAGE_KEY) || "[]"
		);
		localStorageVirtualCohorts = localStorageVirtualCohorts.filter(
			x => x.virtualCohortID !== virtualCohortID
		);
		localStorage.setItem(
			VIRTUAL_COHORTS_LOCAL_STORAGE_KEY,
			JSON.stringify(localStorageVirtualCohorts)
		);
	}
	public static get() {
		return JSON.parse(
