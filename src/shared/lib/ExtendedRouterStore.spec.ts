...
describe("test", () => {
	...
	it("router store updateroute method deletes parameters, alters existing params and adds new ones", () => {
		...
		assert.equal(
			mockInstance.push.args[0][0],
			"/patient?param2=altered&param3=new"
		);
	});
	...
	it("router store updateroute method updates path if one is passed, otherwise, retains existing path", () => {
		...
		assert.equal(
			mockInstance.push.args[0][0],
			"/donkey?param2=altered&param3=new"
		);

		ExtendedRouterStore.prototype.updateRoute.call(
			mockInstance,
			{
				param2: "a",
				param3: "b"
		...
		assert.equal(
			mockInstance.push.args[1][0],
			"/one/two?param1=1&param2=a&param3=b"
		);
	});
	...
	it("router store updateroute method updates path if one is passed, otherwise, retains existing path", () => {
		...
		assert.equal(
			mockInstance.push.args[0][0],
			"/donkey?param2=altered&param3=one%26two"
		);
