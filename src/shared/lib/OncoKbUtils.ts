...
export function extractPmids(evidence: any) {
	...
	if (
		evidence.mutationEffect &&
		evidence.mutationEffect.refs &&
		evidence.mutationEffect.refs.length > 0
	) {
		refs = refs.concat(
			evidence.mutationEffect.refs.map((article: any) => {
				return Number(article.pmid);
			})
		);
	}

	if (evidence.oncogenicRefs && evidence.oncogenicRefs.length > 0) {
		refs = refs.concat(
			evidence.oncogenicRefs.map((article: any) => {
				return Number(article.pmid);
			})
		);
	}
	...
	if (evidences && evidences.length > 0) {
		evidences.forEach(function(record) {
			...
			evidenceArr.forEach(function(evidence: any) {
				...
				if (evidence.evidenceType === "GENE_SUMMARY") {
					...
					if (["LEVEL_0"].indexOf(evidence.levelOfEvidence) === -1) {
						...
						if (
							_treatment.level === "LEVEL_1" ||
							_treatment.level === "LEVEL_2A"
						) {
			...
			if (hasHigherLevelEvidence) {
				sensitivityTreatments.forEach(function(
					treatment: any,
					index: number
				) {
			...
			datum.treatments.sensitivity.forEach(function(
				treatment: any,
				index: number
			) {
				if (treatment.level === "LEVEL_2B") {
					...
					treatment.level === "LEVEL_2A" ||
					treatment.level === "LEVEL_1"
				) {
					datum.drugs.sensitivity.current.push(treatment);
				}
			});
			datum.treatments.resistance.forEach(function(
				treatment: any,
				index: number
			) {
...
export function getTumorTypeFromEvidence(evidence: any) {
	...
	if (_.isObject(evidence.oncoTreeType)) {
		oncoTreeTumorType = evidence.oncoTreeType.subtype
			? evidence.oncoTreeType.subtype
			: evidence.oncoTreeType.cancerType;
	}
...
export function generateMutationEffectCitations(
	mutationEffectRefs: any
): number[] {
...
export function generateTreatments(evidenceTreatments: any) {
	...
	_.each(
		_.keys(treatments).sort(function(a, b) {
