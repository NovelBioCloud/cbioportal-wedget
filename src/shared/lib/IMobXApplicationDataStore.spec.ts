...
import { SimpleMobXApplicationDataStore } from "./IMobXApplicationDataStore";

describe("SimpleMobXApplicationDataStore", () => {
	let data: number[];
	...
	it("initializes with given data, and returns proper defaults", () => {
		let store = new SimpleMobXApplicationDataStore<number>(data);
		assert.deepEqual(
			store.allData,
			data,
