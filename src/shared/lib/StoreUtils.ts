...
export async function fetchStudiesForSamplesWithoutCancerTypeClinicalData(
	...
	if (samplesWithoutClinicalData.result) {
		...
		const promises = studyIdsForSamplesWithoutClinicalData.map(studyId =>
			client.getStudyUsingGET({ studyId })
		);
...
export async function fetchOncoKbAnnotatedGenes(
	client: OncoKbAPI = oncokbClient
): Promise<{ [entrezGeneId: number]: boolean }> {
	return _.reduce(
		await client.genesGetUsingGET({}),
		(map: { [entrezGeneId: number]: boolean }, next: Gene) => {
...
export async function fetchCnaOncoKbDataWithGeneMolecularData(
	...
	if (!geneMolecularData.result || geneMolecularData.result.length === 0) {
		...
		const queryVariants = _.uniqBy(
			_.map(alterationsToQuery, (datum: GeneMolecularData) => {
				return generateQueryVariant(
					datum.entrezGeneId,
					cancerTypeForOncoKb(
...
export async function fetchHotspotsData(
	...
	const queryGenes: string[] = _.uniq(
		_.map(mutationDataResult, function(mutation: Mutation) {
...
export function indexHotspotData(
	hotspotData: MobxPromise<ICancerHotspotData>
): IHotspotData | undefined {
