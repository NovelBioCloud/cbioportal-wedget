...
export default class CBioPortalAPIInternal {
	...
	fetchCosmicCountsUsingPOSTURL(parameters: { keywords: Array<string>; $queryParameters?: any }): string {
		let queryParameters: any = {};
		let path = "/cosmic-counts/fetch";

		if (parameters.$queryParameters) {
			Object.keys(parameters.$queryParameters).forEach(function(parameterName: string) {
	...
	fetchCosmicCountsUsingPOST(parameters: {
		...
		return new Promise(function(resolve, reject) {
			...
			if (parameters.$queryParameters) {
				Object.keys(parameters.$queryParameters).forEach(function(parameterName: any) {
					var parameter = parameters.$queryParameters[parameterName];
					queryParameters[parameterName] = parameter;
				});
			}
	...
	/**
	 * Get gene set hierarchical organization information.
	 *  I.e. how different gene sets relate to other gene sets, in a hierarchy
	 * @method
	 * @name CBioPortalAPIInternal#fetchGenesetHierarchyInfoUsingPOST
	 * @param {string} geneticProfileId - Genetic Profile ID e.g.
	 * gbm_tcga_gsva_scores. The final hierarchy  will only include gene sets scored in the specified profile.
	 * @param {integer} percentile - Percentile (for score calculation).
	 * Which percentile to use when determining the *representative score*
	 * @param {number} scoreThreshold - Gene set score threshold
	 * (for absolute score value). Filters out gene sets where the GSVA(like)
	 * *representative score* is under this threshold.
	 * @param {number} pvalueThreshold - p-value threshold.
	 * Filters out gene sets for which the score p-value is higher than this threshold.
	 * @param {string} sampleListId - Identifier of pre-defined sample list
	 * with samples to query, e.g. brca_tcga_all
	 * @param {} sampleIds - Fill this one if you want to specify a subset
	 * of samples: sampleIds: custom list of samples or patients to query, e.g. ["TCGA-A1-A0SD-01", "TCGA-A1-A0SE-01"]
	 */
	...
	fetchGenesetsUsingPOSTURL(parameters: { genesetIds: Array<string>; $queryParameters?: any }): string {
		let queryParameters: any = {};
	...
	getGenesetUsingGETURL(parameters: { genesetId: string; $queryParameters?: any }): string {
		let queryParameters: any = {};
	 ...
	 */
	getGenesetUsingGET(parameters: { genesetId: string; $queryParameters?: any; $domain?: string }): Promise<Geneset> {
		const domain = parameters.$domain ? parameters.$domain : this.domain;
		const errorHandlers = this.errorHandlers;
		const request = this.request;
		let path = "/genesets/{genesetId}";
	...
	/**
	 * Get the genes in a gene set that have expression correlated to
	 * the gene set scores (calculated using Spearman's correlation)
	 * @method
	 ...
	 * @param {} sampleIds - Fill this one if you want to specify a subset of samples:
	 ...
	 * @param {string} geneticProfileId - Genetic profile ID, e.g. gbm_tcga_gsva_scores
	 * @param {} genesetDataFilterCriteria - Search criteria to return the
	 *  values for a given set of samples and gene set items. genesetIds: 
	 * The list of identifiers for the gene sets of interest, 
	 * e.g. HINATA_NFKB_MATRIX. Use one of these if you want to specify a subset of
	 * samples:(1) sampleListId: Identifier of pre-defined sample list 
	 * with samples to query, e.g. brca_tcga_all or (2) sampleIds: 
	 * custom list of samples or patients to query, e.g. TCGA-BH-A1EO-01, TCGA-AR-A1AR-01
	 */
	 ...
	 */
	getInfoUsingGET(parameters: { $queryParameters?: any; $domain?: string }): Promise<Info> {
		const domain = parameters.$domain ? parameters.$domain : this.domain;
		const errorHandlers = this.errorHandlers;
		const request = this.request;
		let path = "/info";
