...
export default class DatasetPage extends React.Component<{}, {}> {
	...
	public render() {
		if (this.store.data.isComplete) {
			const header: JSX.Element | null = AppConfig.skinDatasetHeader ? (
				<p
					style={{ marginBottom: "20px" }}
					dangerouslySetInnerHTML={{
						__html: AppConfig.skinDatasetHeader
					}}
				/>
			) : null;
			const footer: JSX.Element | null = AppConfig.skinDatasetFooter ? (
				<p
					style={{ marginTop: "20px" }}
					dangerouslySetInnerHTML={{
						__html: AppConfig.skinDatasetFooter
					}}
				/>
			) : null;
			return (
