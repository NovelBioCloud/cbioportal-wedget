...
class CNASegmentIframe extends React.Component<CNSegmentsIframeProps, {}> {
	...
	private getIframeBody() {
		...
		var bodyContent1 =
			'<body style="margin:0"><div id="igvDiv" style="padding-top: 10px;padding-bottom: 10px;"></div><script type="text/javascript">  $(document).ready(function () {    var div = $("#igvDiv"),   options = {' +
			'divId: "igvDiv", showNavigation: true, showRuler: true, genome: "hg19", divId: "igvDiv", locus: "' +
			this.props.gene.hugoGeneSymbol +
			'", tracks: [  { url: "api-legacy/copynumbersegments", indexed: false, isLog: true, contentType: "application/x-www-form-urlencoded", name: "Alt click to sort", type:"seg", json: true, method: "POST", ';
...
export default class CNSegments extends React.Component<{ store: ResultsViewPageStore }, {}> {
	...
	render() {
		if (
			...
			this.props.store.studyIds.isComplete
		) {
			return (
				<MSKTabs
					...
					{this.props.store.genes.result!.map((gene: Gene) => {
						return (
							<MSKTab
								...
								id={`CNSegmentsTab${gene.hugoGeneSymbol}`}
