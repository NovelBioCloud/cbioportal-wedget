...
export class PatientViewPageStore {
	...
	@computed
	get mergedDiscreteCNAData(): DiscreteCopyNumberData[][] {
	...
	readonly darwinUrl = remoteData({
		await: () => [this.derivedPatientId],
		invoke: async () => {
			let enableDarwin: boolean | null | undefined =
				AppConfig.enableDarwin;
	...
	@computed
	get indexedHotspotData(): IHotspotData | undefined {
		return indexHotspotData(this.hotspotData);
	}

	@computed
	get mergedMutationData(): Mutation[][] {
		return mergeMutations(this.mutationData);
	}

	@computed
	get mergedMutationDataIncludingUncalled(): Mutation[][] {
	...
	@computed
	get uniqueSampleKeyToTumorType(): { [sampleId: string]: string } {
